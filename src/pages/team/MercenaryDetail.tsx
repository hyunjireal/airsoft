...
export function MercenaryDetail() {
  ...
  if (!post) {
    return <div className="page"><h1 className="page-title">모집글을 찾을 수 없어요</h1></div>
  }
  ...
  return (
    <div className="page">
      ...
      <section className="section">
        ...
        <article className="card"><h2>태그</h2><div className="chip-row">{post.tags.map((tag) => <span className="chip" key={tag}>{tag}</span>)}</div></article>
      </section>
      <button className="button primary-button" type="button" onClick={() => setModalOpen(true)}>참여 요청하기</button>
