...
export function BoardList() {
  ...
  return (
    <div className="page">
      <h1 className="page-title">{boardNames[typedBoard] ?? '게시판'}</h1>
      <section className="section">
        <input className="input" placeholder="검색" />
        <div className="chip-row"><span className="chip">전체</span><span className="chip">초보</span><span className="chip">인기</span></div>
        <button className="button primary-button" type="button" onClick={write}>글쓰기</button>
      </section>
      <section className="section">
        {posts.map((post) => (
          <Link className="card" key={post.id} to={`/community/post/${post.id}`}>
            ...
            <div className="chip-row">{post.tags.map((tag) => <span className="chip" key={tag}>{tag}</span>)}</div>
          </Link>
        ))}
      </section>
