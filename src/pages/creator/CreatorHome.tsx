...
export function CreatorHome() {
  ...
  return (
    <div className="page">
      <h1 className="page_title">크리에이터</h1>

      <section className="section">
        <div className="card_row">
          <span className="badge">랭킹</span>
          <h2 className="section_title">크리에이터 랭킹</h2>
        </div>
        <div className="creator_rank_row">
          {rankedCreators.map((creator) => (
            <Link className="creator_rank_item" key={creator.id} to={`/creator/${creator.id}`}>
      ...
      <section className="section">
        <div className="card_row">
          ...
          <h2 className="section_title">미디어 컨텐츠</h2>
        </div>
        {mediaContents.map((content) => {
          const creator = creators.find((item) => item.id === content.creatorId)
          return (
            <Link className="card media_card" key={content.id} to={`/creator/${content.creatorId}`}>
              <div className="placeholder_image">영상 하이라이트 썸네일</div>
              <h2>{content.title}</h2>
              <p>{content.summary}</p>
